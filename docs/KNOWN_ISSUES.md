...
天鳳の牌譜は現在、機械学習用途で使用できなくなった。そのため、自前のログ形式が決まるまで、本ドキュメントに記載した問題の修正は **保留** とする。本ドキュメントは問題の存在を記録することを目的とする。

- 調査日: 2026-09-25
- 基準コミット: `7dfbb68`（記載の行番号はすべて基準コミット時点のもの）
- 状態: 全項目 ⏸ 保留

§3 の Rules（`MjlogReplayer/Rules/`）は `GameState` を入力とし、ログ形式には依存しない。自前ログ形式でも `GameState` / Rules を再利用する場合は、修正が必須となる。
...
Rules はログ形式に依存しない。自前ログ形式でも `GameState` / Rules を再利用する場合は、修正が必須となる。
