...
public struct KernelCall : IFlatbufferObject
{
  ...
  public KernelCallT UnPack() {
    var _o = new KernelCallT();
    this.UnPackTo(_o);
    return _o;
  }
  public void UnPackTo(KernelCallT _o) {
    _o.OpIndex = this.OpIndex;
    _o.Args = new List<int>();
    for (var _j = 0; _j < this.ArgsLength; ++_j) {_o.Args.Add(this.Args(_j));}
  }
  public static Offset<SentisFlatBuffer.KernelCall> Pack(FlatBufferBuilder builder, KernelCallT _o) {
    if (_o == null) return default(Offset<SentisFlatBuffer.KernelCall>);
    var _args = default(VectorOffset);
    if (_o.Args != null) {
      var __args = _o.Args.ToArray();
      _args = CreateArgsVector(builder, __args);
    }
    return CreateKernelCall(
      builder,
      _o.OpIndex,
      _args);
  }
