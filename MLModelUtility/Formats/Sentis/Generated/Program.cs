...
public struct Program : IFlatbufferObject
{
  ...
  public ProgramT UnPack() {
    var _o = new ProgramT();
    this.UnPackTo(_o);
    return _o;
  }
  public void UnPackTo(ProgramT _o) {
    _o.Version = this.Version;
    _o.ExecutionPlan = this.ExecutionPlan.HasValue ? this.ExecutionPlan.Value.UnPack() : null;
    _o.SegmentsOffset = this.SegmentsOffset;
    _o.Segments = new List<SentisFlatBuffer.DataSegmentT>();
    for (var _j = 0; _j < this.SegmentsLength; ++_j) {_o.Segments.Add(this.Segments(_j).HasValue ? this.Segments(_j).Value.UnPack() : null);}
  }
  public static Offset<SentisFlatBuffer.Program> Pack(FlatBufferBuilder builder, ProgramT _o) {
    if (_o == null) return default(Offset<SentisFlatBuffer.Program>);
    var _execution_plan = _o.ExecutionPlan == null ? default(Offset<SentisFlatBuffer.ExecutionPlan>) : SentisFlatBuffer.ExecutionPlan.Pack(builder, _o.ExecutionPlan);
    var _segments = default(VectorOffset);
    if (_o.Segments != null) {
      var __segments = new Offset<SentisFlatBuffer.DataSegment>[_o.Segments.Count];
      for (var _j = 0; _j < __segments.Length; ++_j) { __segments[_j] = SentisFlatBuffer.DataSegment.Pack(builder, _o.Segments[_j]); }
...
public class ProgramT
{
  ...
  public SentisFlatBuffer.ExecutionPlanT ExecutionPlan { get; set; }
  public uint SegmentsOffset { get; set; }
  public List<SentisFlatBuffer.DataSegmentT> Segments { get; set; }

  public ProgramT() {
    this.Version = 0;
    this.ExecutionPlan = null;
    this.SegmentsOffset = 0;
    this.Segments = null;
  }
  public static ProgramT DeserializeFromBinary(byte[] fbBuffer) {
    return Program.GetRootAsProgram(new ByteBuffer(fbBuffer)).UnPack();
  }
  public byte[] SerializeToBinary() {
    var fbb = new FlatBufferBuilder(0x10000);
    Program.FinishProgramBuffer(fbb, Program.Pack(fbb, this));
