...
public struct BackendPartitioning : IFlatbufferObject
{
  ...
  public BackendPartitioningT UnPack() {
    var _o = new BackendPartitioningT();
    this.UnPackTo(_o);
    return _o;
  }
  public void UnPackTo(BackendPartitioningT _o) {
    _o.Chains = new List<int>();
    for (var _j = 0; _j < this.ChainsLength; ++_j) {_o.Chains.Add(this.Chains(_j));}
    _o.Backend = this.Backend;
  }
  public static Offset<SentisFlatBuffer.BackendPartitioning> Pack(FlatBufferBuilder builder, BackendPartitioningT _o) {
    if (_o == null) return default(Offset<SentisFlatBuffer.BackendPartitioning>);
    var _chains = default(VectorOffset);
    if (_o.Chains != null) {
      var __chains = _o.Chains.ToArray();
      _chains = CreateChainsVector(builder, __chains);
    }
    return CreateBackendPartitioning(
      builder,
      _chains,
      _o.Backend);
  }
...
public class BackendPartitioningT
{
  public List<int> Chains { get; set; }
  public SentisFlatBuffer.BackendType Backend { get; set; }

  public BackendPartitioningT() {
    this.Chains = null;
    this.Backend = SentisFlatBuffer.BackendType.CPU;
  }
}
